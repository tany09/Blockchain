...
def mine_block():
    ...
    blockchain.add_transactions(sender = node_address, receiver = 'rajul', amount = 1)
    block = blockchain.create_block(proof, previous_hash)
    response = {'message': 'Congratulations you have succesfully mined a new block',
                'index' : block['index'],
...
def is_valid():
    ...
    if is_valid:
        response = {'message' : 'The requested blockchain is valid'}
    else:
...
def connect_node():
    ...
    response ={'message' : 'All nodes are successfully connected. The duckcoin network contains the following nodes',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message' : 'The chain is succesfully replaced by the longest chain',
